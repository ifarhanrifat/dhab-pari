...
interface Project { id: string; title: string; title_ur: string | null; description: string | null; description_ur: string | null; status: string; progress_percent: number; budget_pkr: number | null; spent_pkr: number | null; category: string | null; location: string | null; location_ur: string | null; sector: string | null; is_featured: boolean; before_image_url: string | null; after_image_url: string | null; start_date: string | null; end_date: string | null; beneficiaries_count: number | null; funding_model: string | null; monthly_operating_cost_pkr: number | null
  ...
  hide_donations: boolean; hide_expenses: boolean; hide_donor_names: boolean; hide_fees: boolean
          ...
          <div key={p.id} className="bg-white border border-dp-outline-variant rounded-lg p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-dp-secondary transition-all">
            <div className="flex-1 min-w-0 flex items-start gap-3">
              ...
              <div className="flex items-center gap-3 mb-1 flex-wrap">
                ...
                {!p.is_private && (p.hide_donor_names || p.hide_donations || p.hide_expenses || p.hide_fees) && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold text-purple-700 bg-purple-50 rounded-full px-2 py-0.5 font-sans" title={[p.hide_donor_names && t('pj.hideDonorNames'), p.hide_donations && t('pj.hideDonations'), p.hide_expenses && t('pj.hideExpenses'), p.hide_fees && t('pj.hideFees')].filter(Boolean).join(' · ')}>
                    <Lock size={11} /> {t('pj.privacyOptionsTitle')}
                  </span>
                )}
              </div>
              <h3 className="font-sans text-[18px] font-bold text-dp-on-surface truncate">{p.title}</h3>
              <p className="font-sans text-[14px] text-dp-on-surface-variant">{t(categoryLabelKey[p.category ?? ''] ?? p.category ?? '', p.category ?? '')} · {isUrdu ? (p.location_ur || p.location) : p.location} · <span className="ltr-num">{p.progress_percent}%</span></p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div><label className="block font-sans text-[14px] font-semibold tracking-[0.05em] text-dp-on-surface-variant mb-2">{t('a.titleUr')}</label><input value={form.title_ur} onChange={(e) => setForm({ ...form, title_ur: e.target.value })} className="input-field" style={{ fontFamily: 'var(--font-urdu-ui)', direction: 'rtl' }} /></div>
              ...
              {/* Optional — an English-only Location shows as-is in Urdu mode
                  too (same fallback title_ur/description_ur already use).
                  Fill this in only when the venue name itself needs to read
                  in Urdu, e.g. "Village Community Hall" → "گاؤں کا کمیونٹی ہال". */}
              <div><label className="block font-sans text-[14px] font-semibold tracking-[0.05em] text-dp-on-surface-variant mb-2">{t('pj.locationUr')}</label><input value={form.location_ur} onChange={(e) => setForm({ ...form, location_ur: e.target.value })} className="input-field" style={{ fontFamily: 'var(--font-urdu-ui)', direction: 'rtl' }} /></div>
              ...
              <div className="border border-dp-outline-variant rounded-lg p-3.5 space-y-2.5">
                ...
                {feeChargingCategory(form.category) && (
                  <div>
                    <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={form.hide_fees} disabled={form.is_private} onChange={(e) => setForm({ ...form, hide_fees: e.target.checked })} className="accent-dp-secondary" /><span className="font-sans text-[14px]">{t('pj.hideFees')}</span></label>
                    <p className="font-sans text-[12px] text-dp-on-surface-variant mt-1 ms-6">{t('pj.hideFeesHint')}</p>
                  </div>
                )}
