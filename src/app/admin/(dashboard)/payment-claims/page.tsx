...
export default function AdminPaymentClaimsPage() {
  ...
  if (!access.canWaterSupply) {
    return (
      <div className="bg-white rounded-lg border border-dp-outline-variant p-8 text-center">
        <p className="font-sans text-[14px] text-dp-on-surface-variant">Payment Claims belongs to the Water Supply system — your account doesn&apos;t have access to it.</p>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      {loading ? (
        <p className="text-center py-12 text-dp-on-surface-variant font-sans">Loading...</p>
      ) : claims.length === 0 ? (
        <div className="bg-white rounded-lg border border-dp-outline-variant p-8 text-center">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="font-sans text-[13px] text-dp-on-surface-variant mt-0.5">
                      {c.bills ? `${monthName(c.bills.month)} ${c.bills.year}` : 'Bill'} {c.bills?.bill_number ? `(${c.bills.bill_number})` : ''} · {c.payment_method}
                    </p>
                  </div>
