...
// Donors & Projects field collectors — same "cash held in their name until
// settled" pattern as /admin/collectors (Water Supply), just scoped to
// donors_projects (migration 119: ensure_collector_account() parameterized,
// collector_settlements.system, get_field_collectors_by_system()). Kept as
// its own page rather than parameterizing /admin/collectors itself, so the
// Water Supply page and its behavior stay completely untouched.
  ...
  if (!access.canDonorsProjects) {
    return (
      <div className="bg-white rounded-lg border border-dp-outline-variant p-8 text-center">
        <p className="font-sans text-[14px] text-dp-on-surface-variant">Donor Collectors belongs to the Donors &amp; Projects system — your account doesn&apos;t have access to it.</p>
      </div>
    )
  }
    ...
    <>
      <div className="mb-6">
        <h1 className="font-heading text-[28px] font-bold leading-[36px] text-dp-primary flex items-center gap-2.5">
          <Coins size={26} /> Donor Collectors
        </h1>
        <p className="font-sans text-[13.5px] text-dp-on-surface-variant mt-1">Donations collected on the spot by field collectors, pending remittance.</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-dp-outline-variant overflow-hidden mb-6">
        ...
        {loading ? (
          ...
          <p className="px-5 py-6 text-center font-sans text-[13.5px] text-dp-on-surface-variant">No donor collectors set up yet — grant collector access from User Management.</p>
        ) : (
