...
export async function POST(req: NextRequest) {
  ...
  const [{ data: aiSettings }, { data: products }] = await Promise.all([
    ...
    admin.from('shop_products').select('id, name, name_ur, company').eq('shop_id', shopId).eq('is_active', true).order('name'),
  ])
  ...
  const listing = products.map((p, i) => `${i + 1}. ${p.name}${p.name_ur ? ' / ' + p.name_ur : ''}${p.company ? ' (' + p.company + ')' : ''}`).join('\n')
...
Look at the photo and decide which numbered product (if any) it is. Only pick a number if you are reasonably confident — this affects a real bill. If it doesn't clearly match any item on the list, or you're unsure, say no match.
...
Respond with ONLY a JSON object, no markdown fences, no other text: {"match": <number from the list, or null if no confident match>}`
  ...
  try {
    ...
    return NextResponse.json({ product: { id: product.id, name: product.name, name_ur: product.name_ur, company: product.company } })
  } catch (err) {
