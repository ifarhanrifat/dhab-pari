...
export default function AchievementsPage() {
  ...
  return (
    <div className="max-w-[900px] mx-auto px-6 md:px-12 py-10 min-h-screen">
      <div className="mb-10">
        <h1 className="font-heading text-[32px] font-bold leading-[40px] text-dp-on-surface flex items-center gap-3"><Trophy size={28} className="text-dp-secondary" /> Our Achievements</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-dp-on-surface-variant font-sans text-[16px]">Nothing completed yet — check back soon.</div>
      ) : (
          ...
          {items.map((a) => (
            <div key={a.id} className="bg-white border border-dp-outline-variant rounded-lg p-4 flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                {a.is_private ? (
                  <p className="font-sans text-[14.5px] text-dp-on-surface-variant italic">Private task completed by <span className="font-semibold not-italic">{a.done_by_name ?? 'a committee member'}</span></p>
                ) : (
                  <>
                    <p className="font-sans text-[15px] text-dp-on-surface" dir="rtl" style={{ fontFamily: 'var(--font-urdu), serif' }}>{a.text_ur}</p>
                    <p className="font-sans text-[12.5px] text-dp-on-surface-variant mt-1">Completed by <span className="font-semibold">{a.done_by_name ?? 'a committee member'}</span></p>
                  </>
                )}
