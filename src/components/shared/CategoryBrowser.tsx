...
// Department → category tile browser for a shop's OWN existing catalog.
// A shop keeper taps a department tile (icon), then a category tile
// inside it, and lands on that category's own product grid with "Add
// Item" already scoped to it — no dropdown anywhere. Shared between
// admin/shops (staff-managed catalog) and portal/my-shop (keeper
// self-service) since both need the exact same browse shape, just with
// their own product-card look and add/edit/delete wiring passed in as
// props.
//
...
//
// Bulk-populating from the reference catalog (ticking many items, then a
// single price-review pass) is a separate flow — see AddStockWizard.tsx —
// not this component. Brand used to be a navigation level for picking new
// items here; it's cut from that role (see AddStockWizard's header
// comment for why) and survives only as a filter chip over a category's
// own already-stocked products, once there's more than one company on
// the shelf worth telling apart.

import { useMemo, useState } from 'react'
import { ChevronRight, Search, PlusCircle, LayoutGrid, Tags } from 'lucide-react'
...
interface CategoryBrowserProps<P extends { name: string; name_ur?: string | null; company?: string | null; category: string | null }> {
  primaryType: string
  products: P[]
...
export function CategoryBrowser<P extends { name: string; name_ur?: string | null; company?: string | null; category: string | null }>({
  ...
  // Small, already category-scoped lists — plain values, not memoized;
  // wrapping trivial filters over a single category's own products in
  // useMemo here bought nothing and confused the React Compiler about
  // activeCat's stability.
  const productsInCat = activeCat ? products.filter((p) => p.category === activeCat.slug) : []
  // "Brands" as a lens on what's already on the shelf — a reorder/
  // price-change question ("show me everything I stock from Nestlé"),
  // not a way to find new items. Only worth showing once there's
  // actually more than one company to tell apart.
  const brandNames = new Set<string>()
  for (const p of productsInCat) if (p.company?.trim()) brandNames.add(p.company.trim())
  const brandsInCat = [...brandNames].sort()
  const visibleInCat = brandFilter ? productsInCat.filter((p) => (p.company ?? '').trim() === brandFilter) : productsInCat
  ...
  const goDepartments = () => { setActiveDeptKey(null); setActiveCatSlug(null); setBrandFilter(null) }
  const goCategories = (deptKey: string) => { setActiveDeptKey(deptKey); setActiveCatSlug(null); setBrandFilter(null) }
  const openCategory = (slug: string) => { setActiveCatSlug(slug); setBrandFilter(null) }
        ...
        <>
          ...
          {brandsInCat.length > 1 && (
            <div className="flex items-center gap-1.5 mb-4 overflow-x-auto pb-1">
              <Tags size={13} className="shrink-0 text-dp-on-surface-variant" />
              <button onClick={() => setBrandFilter(null)} className={`shrink-0 px-2.5 py-1 rounded-full text-[11.5px] font-sans font-semibold cursor-pointer border ${!brandFilter ? 'bg-dp-secondary text-white border-dp-secondary' : 'bg-white text-dp-on-surface-variant border-dp-outline-variant'}`}>{t('cb.allTab')}</button>
              {brandsInCat.map((b) => (
                <button key={b} onClick={() => setBrandFilter(b)} className={`shrink-0 px-2.5 py-1 rounded-full text-[11.5px] font-sans font-semibold cursor-pointer border ${brandFilter === b ? 'bg-dp-secondary text-white border-dp-secondary' : 'bg-white text-dp-on-surface-variant border-dp-outline-variant'}`}>{b}</button>
              ))}
            </div>
          )}
          ...
          {productsInCat.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {visibleInCat.map(renderProduct)}
            </div>
          )}
        </>
      )}
