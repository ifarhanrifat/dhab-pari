-- Migration 242: my_pool_commitments()/my_pool_announcements() (migration
-- 236) learned to show a named Kafalat child or Wazifa student, but never a
-- named Sadqa object — migration 237 added sadqa_object_id to the same two
-- tables without also touching these two read functions. The donor portal's
-- new upkeep section needs "named_object" the same way it already gets
-- "named_child"/"named_student".
