-- Migration 166: fix get_transactions_workspace_documents (164) —
-- inventory_transactions' FK to inventory_items is item_id, not
-- inventory_item_id (that name only exists on connection_template_items, a
-- different table). CREATE FUNCTION doesn't validate embedded SQL until
-- it's actually executed, so 164 applied "successfully" with this latent
-- bug — caught here via the debug-wrapper verification pass (165) before
-- any client code depended on it.
