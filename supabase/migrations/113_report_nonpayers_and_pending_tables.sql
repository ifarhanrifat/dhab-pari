-- Migration 113: auto-detected non-payers-due-to-complaint (replacing the old
-- manually-typed free-text list — complaints.consumer_id, migration 097,
-- already gives a reliable link, no name-matching needed), plus per-consumer
-- pending-bills detail (previously only sector totals). The old `non_payers`
-- column and update_closing_report_non_payers() are left in place unused
-- rather than dropped — same "stop using, don't delete" precedent as WS-4004.

ALTER TABLE monthly_closing_reports ADD COLUMN IF NOT EXISTS non_payers_due_to_complaint jsonb DEFAULT '[]';
-- The one human-entered part — the committee's decision on each case — keyed
-- by consumer_id so it survives a regenerate the same way reconciliation_remarks does.
ALTER TABLE monthly_closing_reports ADD COLUMN IF NOT EXISTS non_payer_opinions jsonb NOT NULL DEFAULT '[]';
ALTER TABLE monthly_closing_reports ADD COLUMN IF NOT EXISTS pending_bills_by_consumer jsonb DEFAULT '[]';
...
BEGIN
  ...
  IF p_system = 'water_supply' THEN
    ...
    -- Per-consumer detail behind the sector totals above — same balance
    -- calc, just not collapsed by sector.
    ...
    -- Consumers with an active (not yet fully verified) complaint AND an
    -- outstanding bill balance — reliable via complaints.consumer_id
    -- (migration 097), not name-matching.
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
...
CREATE OR REPLACE FUNCTION update_closing_report_non_payer_opinions(p_report_id uuid, p_opinions jsonb) RETURNS void AS $$
DECLARE
  v_system varchar;
BEGIN
...
REVOKE ALL ON FUNCTION update_closing_report_non_payer_opinions(uuid, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION update_closing_report_non_payer_opinions(uuid, jsonb) TO authenticated;
