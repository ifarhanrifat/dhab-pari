-- Migration 112: three real bugs in the monthly closing report, caught by
-- the user reviewing Running Capital:
--
-- 1. "Discount Given" (WS-3008) and "Discount on Sale" (WS-3009) are typed
--    'expense' in the chart of accounts (a bookkeeping convenience so they
--    debit/credit like any other expense-normal account — see migrations
--    035/067), so the report's blanket "WHERE a.type = 'expense'" swept them
--    into Total Expenses / the expense table right alongside real cash
--    expenses. They're revenue reductions, not expenses — excluded now.
-- 2. Purchases (ledger reference_type='inventory', txn_type='purchase')
--    fell into the cash_out_breakdown's generic 'Other' bucket instead of
--    being labeled — now call out "Purchases" specifically.
-- 3. complaints_this_month only ever carried {name, sector, text} — no
--    status, no incharge, no resolution info, so a resolved/verified
--    complaint looked identical to a brand new one. Enriched with status,
--    incharge (assigned_to) name, and resolved-by name/date.
--
-- Existing already-generated report rows keep their old JSON shape until
-- regenerated — regenerate_monthly_closing_report() below lets an admin
-- refresh a specific past report on demand rather than waiting for next
-- month's cron run (which only ever (re)computes last month).
...
-- Refresh one already-persisted report on demand (the cron only ever
-- (re)computes *last* month — this lets an admin fix any older stale one
-- without waiting), mirrors update_closing_report_non_payers' auth pattern.
...
BEGIN
  ...
  -- reconciliation_remarks/non_payers deliberately untouched — accountant-entered, must survive a regenerate.
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

REVOKE ALL ON FUNCTION regenerate_monthly_closing_report(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION regenerate_monthly_closing_report(uuid) TO authenticated;
