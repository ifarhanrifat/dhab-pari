-- Real report, 2026-09-25: an expense voucher (DP-EXP-V-0438, PKR 10,000
-- paid to Muhammad Arshad Dulyal's medical-expense project) posted
-- correctly and DOES carry vouchers.project_id -- but neither Recent
-- Transactions (Transactions Workspace) nor the All Transactions audit
-- page could show which project it belonged to, because
-- get_transactions_workspace_documents() never selected v.project_id in
-- the first place. Not a data bug -- a display gap. Adding project_id
-- plus the project's own title/title_ur (joined here so the client
-- doesn't need a second round trip) to the vouchers payload only.
