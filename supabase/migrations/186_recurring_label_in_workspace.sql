-- Migration 186: surface which documents were generated by a recurring schedule.
--
-- bills, vouchers and donors all already carry recurring_schedule_id — it was
-- simply never returned to the Transactions Workspace, so an accountant looking
-- at a list had no way to tell a one-off donation from the third instalment of
-- a standing monthly commitment, or a hand-written bill from one the scheduler
-- raised. Same for an announced (unpaid) recurring donation, which is exactly
-- the case the donor accountant most needs to classify.
--
-- payment_status comes along for donations at the same time, so the workspace
-- can tell announced from awaiting-confirmation without a second query.
--
-- Body is otherwise byte-identical to migration 166.
