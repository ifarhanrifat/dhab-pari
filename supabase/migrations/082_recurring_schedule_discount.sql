-- Migration 082: The Activation/Setup-Billing steps were folding a discount
-- straight into a lower recurring_schedules.amount_pkr ("net amount" trick)
-- instead of recording it as a real discount_amount — recurring_schedules had
-- no such column, so there was nowhere else to put it. That silently broke
-- the Billing page's "With Discount" filter (which reads bills.discount_amount)
-- for every schedule-generated bill, and skipped the gross-method posting
-- (migration 067) that splits a discount onto its own WS-3008/WS-3009 ledger
-- leg instead of just shrinking the income line.
ALTER TABLE recurring_schedules ADD COLUMN IF NOT EXISTS discount_amount decimal NOT NULL DEFAULT 0;
