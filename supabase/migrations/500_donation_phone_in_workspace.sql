-- Real bug found+fixed 2026-09-22: the donation receipt's WhatsApp button
-- always fell back to the generic wa.me chat picker (no specific contact
-- pre-selected), even when the donor had a real phone/whatsapp_number on
-- file -- because get_transactions_workspace_documents() never returned
-- either field, so the finance page had nothing to pass through to
-- ReceiptModal's phone prop. Water-supply's equivalent (the bill invoice
-- page) already resolves this correctly from `consumers`; donations were
-- just missing the same field on the donations JSON.
