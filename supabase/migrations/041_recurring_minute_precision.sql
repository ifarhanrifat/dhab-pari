-- Migration 041: recurring_schedules.next_run_date was a plain `date` column, so
-- nothing finer than a whole day was ever meaningful — there was no way to actually
-- verify the recurring engine fires without waiting a full day. Converting it to
-- timestamptz (existing rows become midnight UTC, so daily/weekly/monthly/etc.
-- behavior is unchanged) and adding an 'every_minute' frequency makes the mechanism
-- testable: create a schedule, wait ~60s, reload the Recurring page (which calls
-- run_due_recurring_schedules() as its fallback check) and see it fire.

ALTER TABLE recurring_schedules ALTER COLUMN next_run_date TYPE timestamptz USING next_run_date::timestamptz;
...
ALTER TABLE recurring_schedules ADD CONSTRAINT recurring_schedules_frequency_check
  CHECK (frequency IN ('every_minute', 'daily', 'weekly', 'monthly', 'semi_annual', 'yearly'));
...
BEGIN
  ...
  v_due_date := s.next_run_date::date + COALESCE(s.due_date_offset_days, 7);
