-- Migration 265: the two follow-ups from the Kafalat granular-accounts work.
--
-- 1. A multi-category voucher's "View voucher" receipt only ever showed the
--    lump total -- the same gap purchase_line_items already solved for
--    purchases, never extended to vouchers. get_transactions_workspace_
--    documents() now returns voucher_line_items the same way, so the print/
--    PDF/PNG receipt can show the real category breakdown instead of one
--    unexplained number.
-- 2. edit_voucher() has always refused to touch a voucher with line items
--    (migration 175) -- correct, but the Edit button only found that out
--    when someone clicked Save and got an error. The frontend can now grey
--    it out up front once it knows which vouchers are itemised.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'voucher_line_items', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'voucher_id', li.voucher_id, 'description', li.description, 'category', li.category, 'amount', li.amount
