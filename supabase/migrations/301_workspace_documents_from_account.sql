-- Migration 301: Recent Transactions needs to know which account a
-- multi-line expense was paid from and which account its first line went
-- to, to build a "Cash → Discount on Sale" label instead of the free-text
-- Party Name field (or a generic "Expense" fallback) it showed before —
-- neither of those told anyone which accounts were actually involved
-- without opening the voucher. get_transactions_workspace_documents()
-- (migration 265) didn't return from_account_id on vouchers or account_id
-- on voucher_line_items at all; this only adds those two fields, nothing
-- else changes.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'vouchers', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'party_name', v.party_name, 'from_account_id', v.from_account_id, 'bill_id', v.bill_id, 'created_at', v.created_at,
